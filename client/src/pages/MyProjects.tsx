...
const MyProjects = () => {
    ...
    const fetchProjects = async () => {
      setProjects(dummyProjects); // Replace with actual API call to fetch projects
       //simuulate loading
       setTimeout(() => {
        setLoading(false);
       }, 1000);
        ...
        {loading ? (
          ...
          <div className= 'flex flex-wrap gap-3.5'>
            {projects.map((project) => (
              <div onClick={() => navigate(`/projects/${project.id}`)} key={project.id} className='relative group w-72 max-sm:mx=auto cursor-pointer bg-gray-900/60 boarderborder-gray-700 rounded-lg overflow-hidden shadow-md group hover:shadow-indigo-700/30 houver:border-indigo-800/80 transition-all duration-300'>
                ...
                <div className='relative w-full h-40 bg-gray-900 overflow-hidden border-b border-gray-800'>
                  ...
                  {/* Content */ }
                  <div className='p-4 text-white bg-linear-180 from-transparent group-hover:from-indigo-950 o-transparent transition-colors'>
                    ...
                    <div onClick={(e)=>e.stopPropagation()} className='flex justify-between items-center mt-6'>
                      ...
                      <div className='flex gap-3 text-white text-sm'>
                        <button onClick={()=>navigate(`/projects/${project.id}`)} className='px-3 py-1.5 bg-white/10 hover:bg-white/15 rounded-md transition-colors'>Preview</button>
          ...
          <div className='flex items-center justify-center h-[80vh]'>
            ...
            <button onClick={() => navigate('/')} className='text-white px-5 py-2mt-5 rounded-md bg-indigo-500 hover:bg-indigo-600 active:scale-95 transition-all'>
              Create New
            </button>
